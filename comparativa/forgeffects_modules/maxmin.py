...
def maxmin(tensor1, tensor2):
    ...
    tensor1_expanded = tf.expand_dims(tensor1, axis=3) # O(1)
    tensor2_expanded = tf.expand_dims(tensor2, axis=1) # O(1)

    # Encuentro los mínimos entre los valores de los tensores expandidos

    min_result = tf.minimum(tensor1_expanded, tensor2_expanded) # O(N)

    min_result = tf.transpose(min_result, perm=[0, 1, 3, 2]) # O(1)

    # Encuentro el máximo entre los valores
    max_result = tf.reduce_max(min_result, axis=3) # O(N)
