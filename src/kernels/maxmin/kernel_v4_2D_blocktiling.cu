...
__global__ void maxmin_threshold_kernelv4(
    ...
    // Clave para coalescencia: debemos dar saltos de memoria que cuadren con el total de hilos
    // para que en una pasada hilos contiguos accedan a memoria contigua.
    
    const uint strideA = numThreadsBlocktile / BK; //  cuantas filas de As se cargan en cada parasada
    const uint strideB = numThreadsBlocktile / BN; //  lo mismo pero para Bs
    ...
    for (int bkIdx = 0; bkIdx < Kpad; bkIdx += BK) {
      ...
      for (uint loadOffset = 0; loadOffset < BM; loadOffset += strideA) {
        As[(innerRowA + loadOffset) * BK + innerColA] =
            A[(innerRowA + loadOffset) * Kpad + innerColA];
      }

      for (uint loadOffset = 0; loadOffset < BK; loadOffset += strideB) {
        Bs[(innerRowB + loadOffset) * BN + innerColB] =
            B[(innerRowB + loadOffset) * Npad + innerColB];
      }
