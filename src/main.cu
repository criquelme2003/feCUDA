...
__global__ void randomInit(curandState_t *states, unsigned long long seed,
                           int total_elements) {
...
void test_dims(std::vector<int> ms, std::vector<int> batch_sizes) {
  ...
  for (int M : ms) {
    for (int batch_size : batch_sizes) {
      ...
      CHECK_CUDA(
          cudaMalloc(&curand_state, sizeof(curandState_t) * total_elements));
      ...
      randomInit<<<static_cast<int>(gridSize), static_cast<int>(blockSize)>>>(
          curand_state, RAND_SEED, total_elements);
      CHECK_CUDA(cudaDeviceSynchronize());

      randomFill<<<static_cast<int>(gridSize), static_cast<int>(blockSize)>>>(
          t1.getData(), total_elements, curand_state, RAND_SEED);
      CHECK_CUDA(cudaDeviceSynchronize());
      ...
      // maxmin<__half>(t1, t2, maxres, minres, start, end);
