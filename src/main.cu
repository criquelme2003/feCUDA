...
// Margen de empate: debe coincidir con el usado por el kernel GPU
// (half_equal_with_margin en all_pivots_v1.cu) para que el conteo de pivotes
// sea comparable entre CPU y GPU.
...
bool run_and_validate(int B, int M, float thr, int n_runs = 3) {
    ...
    // Referencia CPU: cuenta pivotes/empates totales por celda (misma
    // semántica que el contador GPU, que suma local_max_count por celda).
    int cpu_count = cpu_maxmin_count(h_A, B, M, thr);
    printf("  CPU reference (pivot count): %d paths\n", cpu_count);
    ...
    for (int run = 0; run < n_runs; run++) {
        ...
        __half hthr = __float2half(thr);
        auto result = maxminv2_count(t1, t2, hthr, 1, "csr_data");
        ...
        if (run > 0 && gpu_count != prev_gpu) {
            printf(
                "  *** VARIACION ENTRE RUNS: run%d=%d vs run%d=%d ***\n",
                run,
                gpu_count,
                run - 1,
                prev_gpu
